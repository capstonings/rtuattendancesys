...
// prevents user to use back button after logout
// if($_SESSION['blocker'] == '')
// {
...
if(isset($_POST['changepass'])) {
	...
	$passquery = "SELECT password FROM user WHERE username = '$uusername'";
	$ppass = mysqli_query($db,$passquery);
	$ppass = mysqli_fetch_assoc($ppass);
	$ppass = reset($ppass);

	if (($ppass != md5($rpassword)) && ($ppass != $rpassword)) {
		array_push($errors1, 'not match'); 
		$_SESSION['data1'] = 'unmatched2';
		$_SESSION['errors1'] = $errors1;
		header("location: change-password.php");
	}
	if (empty($rpassword)) {
		array_push($errors1, 'rpassword is Required'); 
		$_SESSION['data1'] = 'empty';
		$_SESSION['errors1'] = $errors1;
		header("location: change-password.php");
	}
	if (empty($npassword)) {
	...
	{array_push($errors1, 'not match'); 
		$_SESSION['data1'] = 'unmatched1';
		$_SESSION['errors1'] = $errors1;
		header("location: change-password.php");
	}

	if (count($errors1) == 0) {
		$setpassword = md5($npassword);
		$query = "UPDATE user
		SET 
...
## SYSTEM ADMIN Register Function Input -> MYSQL
if (isset($_POST['Register'])) {
	...
	if (count($errors) == 0) {
		...
		echo '<script>alert("DEAN Account Successfully Created!");window.location.href="systemadmin-createacc.php";</script>';
	}
	$_SESSION['errors'] = $errors;
}

## DEAN Register Function Input -> MYSQL
if (isset($_POST['deanRegister'])) {
	...
	$email = "";
	$password = "";
	$accesslevel = mysqli_real_escape_string($db,$_POST['accesslevel']); ;
	$status = 'ACTIVE';
	...
	if (empty($email)) {array_push($errors, 'Email is Required'); $_SESSION['data'] = 'empty';}
	if (empty($password)) {array_push($errors, 'Password is Required'); $_SESSION['data'] = 'empty';}
	if ($accesslevel == "Please Select"){array_push($errors, 'Access Level is Required'); $_SESSION['data'] = 'empty';}
...
if (isset($_POST['Login'])) {
	...
	if (count($errors) == 0) {
		...
		if (mysqli_num_rows($results)) {
			...
			//display status depends on username
			$squery = "SELECT status FROM user WHERE username = '$username'";
			$squery = mysqli_query($db,$squery);
			...
			if($acclevel == 'FACULTY'){
				if($status == "INACTIVE"){
					echo '<script>alert("This Account is inactive.\nPlease Contact The Dean.");window.location.href="login.php";</script>';				
				}
				else{
				...
				if($acclevel == 'DEAN'){
					if($status == "INACTIVE"){
						echo '<script>alert("This Account is inactive.\nPlease Contact The System Administrator.");window.location.href="login.php";</script>';				
					}
					else{
					...
					if($acclevel == 'STUDENT'){
						...
						//display birthday depends on username
						$bday = "SELECT birthday FROM student WHERE username = '$username'";
						$bday = mysqli_query($db,$bday);
						$bday = mysqli_fetch_assoc($bday);
						$bday = reset($bday);
						...
						if($acclevel == 'SYSTEMADMIN'){
							header("location: systemadmin-page.php");
						}
						else{
							if($acclevel == 'PROGRAMCOORDINATOR'){
								header("location: progcoor-page.php");
							}
						}
					}
				}
...
if (isset($_POST["import"])) {
	$dbchecker = "SELECT * FROM student;";
	$dbquery = mysqli_query($db, $dbchecker);
	if (mysqli_num_rows($dbquery) > 0) {
		$c_user = "DELETE from user WHERE accesslevel = 'STUDENT'";
		mysqli_query($db, $c_user);
		$c_compiled_attendance = 'TRUNCATE compiled_attendance';
		mysqli_query($db, $c_compiled_attendance);
		$c_courses_enrolled = 'TRUNCATE courses_enrolled';
		mysqli_query($db, $c_courses_enrolled);
		$c_loaded_subject = 'TRUNCATE loaded_subject';
		mysqli_query($db, $c_loaded_subject);
		$c_recent_schedule = 'TRUNCATE recent_schedule';
		mysqli_query($db, $c_recent_schedule);
		$c_student = 'TRUNCATE student';
		mysqli_query($db, $c_student);
		$c_student_attendance = 'TRUNCATE student_attendance';
		mysqli_query($db, $c_student_attendance);
	}
...
if (isset($_POST['verifystud'])) {
	$student_id = mysqli_real_escape_string($db,$_POST['student_id']);
	$mscannedqr = mysqli_real_escape_string($db,$_POST['scannedqr']);
	$scannedqr = substr($mscannedqr, 0, 10);
	...
	$_SESSION['vstudent_id'] = $student_id . $scannedqr;
	header("location: verify-student.php");
}

if (isset($_POST['cancelverify'])) {
	if($_SESSION['accesslevel'] == "DEAN"){
		header("location: dean-qrscanner.php");
	}
	else{
		header("location: faculty-qrscanner.php");
	}
}
...
if (isset($_POST['selectStud'])) {
	...
	$scannedqr = $_SESSION['vscannedqr'];
	$qrcount = strlen($_SESSION['vscannedqr']);
	if($qrcount == 18){
		$scannedqr = substr($_SESSION['vscannedqr'], 0, 10);
		$bday = substr($_SESSION['vscannedqr'], 10, 8);
		$studbday = "SELECT birthday FROM student WHERE student_id = '$scannedqr'";
		$studbday = mysqli_query($db,$studbday);
		$studbday = mysqli_fetch_assoc($studbday);
		$studbday = reset($studbday);
		if($bday != $studbday){
			if($_SESSION['accesslevel'] == "DEAN"){
				echo '<script>alert("Invalid QR Code!");window.location.href="dean-qrscanner.php";</script>';
				exit();
			}
			else{
				echo '<script>alert("Invalid QR Code!");window.location.href="faculty-qrscanner.php";</script>';
				exit();
			}
		}
	}
	$student_id = $_SESSION['vstudent_id'];
	...
	//student cancellator of already scanned qr
	$studCan = "SELECT * FROM student_attendance WHERE (student_id = '$student_id' OR student_id = '$scannedqr') AND subject = '$subject' AND section = '$section' AND date_of_schedule = '$dateCancellator'";
	$studresult = mysqli_query($db, $studCan);
	if (mysqli_num_rows($studresult) > 0) {
		if($_SESSION['accesslevel'] == "DEAN"){
			echo '<script>alert("Student Already Selected!");window.location.href="dean-scannedqr.php";</script>';
			exit();
		}
		else{
			echo '<script>alert("Student Already Selected!");window.location.href="faculty-scannedqr.php";</script>';
			exit();
		}
	}
...
if (isset($_POST['newSelectStud'])) {
	$student_id = mysqli_real_escape_string($db,$_POST['newstudent_id']);
	$scannedqr = mysqli_real_escape_string($db,$_POST['scannedqr']);
	$scannedqr = $_SESSION['vscannedqr'];
	$qrcount = strlen($_SESSION['vscannedqr']);
	if($qrcount == 18){
	...
	$ssval = mysqli_real_escape_string($db,$_POST['newSelectStud']);
	if((!(empty($student_id))) and (!(empty($scannedqr)))){
		if($ssval == 'Select Student'){
			if($_SESSION['accesslevel'] == "DEAN"){
				echo '<script>alert("Please do not input data in Manual Selector if you want to scan QR Code!");window.location.href="dean-qrscanner.php";</script>';
...
if (isset($_POST["ProfViewer"])){
	$prof_id_viewer = $_POST["ProfViewer"];
	//display lastname depends on username
	$lastquery = "SELECT lastname FROM user WHERE username = '$prof_id_viewer'";
	$lquery = mysqli_query($db,$lastquery);
	$lastnamequery = mysqli_fetch_assoc($lquery);
	$lastname = reset($lastnamequery);
	
	//display firstname depends on username
	$firstquery = "SELECT firstname FROM user WHERE username = '$prof_id_viewer'";
	$fquery = mysqli_query($db,$firstquery);
	$firstnamequery = mysqli_fetch_assoc($fquery);
	$firstname = reset($firstnamequery);
	
	//display middlename depends on username
	$midquery = "SELECT middlename FROM user WHERE username = '$prof_id_viewer'";
	$mquery = mysqli_query($db,$midquery);
	$middlenamequery = mysqli_fetch_assoc($mquery);
	$middlename = reset($middlenamequery);

	//display access level depends on username
	$aacclevel = "SELECT accesslevel FROM user WHERE username = '$prof_id_viewer'";
	$aacclevel = mysqli_query($db,$aacclevel);
	$aacclevel = mysqli_fetch_assoc($aacclevel);
	$aacclevel = reset($aacclevel);

	//display email depends on username
	$aemail = "SELECT email FROM user WHERE username = '$prof_id_viewer'";
	$aemail = mysqli_query($db,$aemail);
	$aemail = mysqli_fetch_assoc($aemail);
	$aemail = reset($aemail);
	
	//display all subjects and section
	$all = "SELECT * FROM loaded_subjects WHERE username = '$prof_id_viewer'";
	$all = mysqli_query($db,$all);
	$all = mysqli_fetch_assoc($all);
	$_SESSION['sched1'] = $all['sched1'];
	$_SESSION['sched2'] = $all['sched2'];
	$_SESSION['sched3'] = $all['sched3'];
	$_SESSION['sched4'] = $all['sched4'];
	$_SESSION['sched5'] = $all['sched5'];
	$_SESSION['sched6'] = $all['sched6'];
	$_SESSION['sched7'] = $all['sched7'];
	$_SESSION['sched8'] = $all['sched8'];
	$_SESSION['sched9'] = $all['sched9'];

	$_SESSION['pusername'] = $prof_id_viewer ;
	$_SESSION['plastname'] = $lastname ;
	$_SESSION['pfirstname'] = $firstname ;
	$_SESSION['pmiddlename'] = $middlename ;
	$_SESSION['pemail'] = $aemail ;
	$_SESSION['paccesslevel'] = $aacclevel ;
	header("location: progcoor-view-prof.php");
...
// drop student subjects buttons until line : 1381 [dean access]
...
if (isset($_POST["dropSubject1"])){
	...
	elseif($_SESSION['accesslevel'] == 'FACULTY'){
		echo '<script>alert("Subject Successfully Dropped!");window.location.href="faculty-student-info.php";</script>';
	}
...
// drop faculty subjects buttons until line : 1473 [program coordinator access]

if (isset($_POST["removeSubject1"])){
    $remSubject = $_SESSION['sched1'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched1 = ''
		WHERE email = '$remProf';";
	mysqli_query($db,$sqlrem);
	echo '<script>alert("Subject Successfully Dropped!");window.location.href="progcoor-page.php";</script>';
}

if (isset($_POST["removeSubject2"])){
	$remSubject = $_SESSION['sched2'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched2 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject3"])){
	$remSubject = $_SESSION['sched3'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched3 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject4"])){
	$remSubject = $_SESSION['sched4'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched4 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject5"])){
	$remSubject = $_SESSION['sched5'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched5 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject6"])){
	$remSubject = $_SESSION['sched6'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched6 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject7"])){
	$remSubject = $_SESSION['sched7'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched7 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject8"])){
	$remSubject = $_SESSION['sched8'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched8 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["removeSubject9"])){
	$remSubject = $_SESSION['sched9'];
	$remProf = mysqli_real_escape_string($db,$_POST['p_email']);
	$sqlrem = "UPDATE loaded_subjects
		SET sched9 = ''
		WHERE email = '$remProf';";
...
if (isset($_POST["addSubject"])){
	$schedsubject= mysqli_real_escape_string($db,$_POST['subject']); 
	$schedsection = mysqli_real_escape_string($db,$_POST['section']);
	// get the current academic year start/end and semester
	$sqlSelect = "SELECT * FROM courses_enrolled";
    $result = mysqli_query($db, $sqlSelect);
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_array($result)) {
			$_SESSION['pays'] = $row['academic_year_start'];
			$_SESSION['paye'] = $row['academic_year_end'];
			$psem = $row['semester'];
			$_SESSION['psem'] = $psem[0];
			break;
		}
	}

	$schedemail = $_SESSION['pemail'];
	$schedInput = $schedsubject . ":" . $schedsection;
	$response = '';
	//checker (ccheck sa lahat ng existing na prof kung existing na ang chosen subject)
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_array($result)) {
			for($i=1;$i<10;$i++){
				...
				$chsched = $row[$schedcounter];
				if($chsched == $schedInput){
					echo '<script>alert("Subject Is Already Existing!");window.location.href="progcoor-page.php";</script>';
					$response = "existing";
					break;
				}
			}
	...
	// schedule adder for selected email if the schedule passed the checker
	if($response == ''){
		$sqladder = "SELECT * FROM loaded_subjects WHERE email = '$schedemail'";
		$result = mysqli_query($db, $sqladder);
		if (mysqli_num_rows($result) > 0) {
			while ($row = mysqli_fetch_array($result)) {
				for($i=1;$i<10;$i++){
					$schedcounter = "sched" . $i;
					$chsched = $row[$schedcounter];
					if($chsched == ''){
						$subjectAdder = "UPDATE loaded_subjects
							SET $schedcounter = '$schedInput'
							WHERE email = '$schedemail';";
						mysqli_query($db, $subjectAdder);
						echo '<script>alert("Subject Successfully Loaded!");window.location.href="progcoor-page.php";</script>';
						break;
					}
				}
			}
		...
		if (mysqli_num_rows($result) <= 0) {
			$all = "SELECT * FROM user WHERE email = '$schedemail'";
			$all = mysqli_query($db,$all);
			$all = mysqli_fetch_assoc($all);
			$newquery = "INSERT INTO loaded_subjects (username,email,firstname,lastname,middlename,sched1,AY_start,AY_end,semester) 
				VALUES ('" . $all['username'] . "','" . $schedemail . "','" . $all['firstname'] . 
				"','" . $all['lastname'] . "','" . $all['middlename'] . "','" . $schedInput . 
				"','" . $_SESSION['pays'] .  "','" . $_SESSION['paye'] .  "','" . $_SESSION['psem'] ."')" ;
...
// dean dashboard 9 buttons on card
if (isset($_POST["schedViewer1"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule1"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
    	$nsection = substr($_SESSION['schedule'], ($splitter+1), $length);
		$_SESSION['usubject'] = $nsubject;
		$_SESSION['usection'] = $nsection;
		header("location: dean-schedule-viewer.php");
	}
	if($_SESSION['accesslevel'] == 'FACULTY'){
		...
		$_SESSION['usubject'] = $nsubject;
		$_SESSION['usection'] = $nsection;
		header("location: faculty-schedule-viewer.php");
	}
}

if (isset($_POST["schedViewer2"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
...
if (isset($_POST["schedViewer3"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule3"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["schedViewer4"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule4"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["schedViewer5"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule5"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["schedViewer6"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule6"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["schedViewer7"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule7"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["schedViewer8"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule8"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["schedViewer9"])){
	if($_SESSION['accesslevel'] == 'DEAN'){
		$schedule = $_POST["schedule9"];
		$_SESSION['schedule'] = $schedule;
		$splitter = strpos($_SESSION['schedule'],":");
    	$length = strlen($_SESSION['schedule']);
    	$nsubject = substr($_SESSION['schedule'], 0, ($splitter));
...
if (isset($_POST["UpdateDeanInfo"])){
	$sysfirstname = mysqli_real_escape_string($db,$_POST['sysfirstname']);
	$sysmiddlename = mysqli_real_escape_string($db,$_POST['sysmiddlename']);
	$syslastname = mysqli_real_escape_string($db,$_POST['syslastname']);
	$sysemail = mysqli_real_escape_string($db,$_POST['sysemail']);
	$sysusername = mysqli_real_escape_string($db,$_POST['sysun']);
	$modifyUser="UPDATE user
					SET 
	...
	mysqli_query($db, $modifyUser);
	session_destroy();
	echo '<script>alert("Success! Please Login Again.");window.location.href="login.php";</script>';
}
?>